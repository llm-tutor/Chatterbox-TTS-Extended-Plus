...
def test_audiostretchy_precision():
    ...
    for speed_factor in speed_factors:
        ...
        try:
            ...
            if processed_audio.dtype == torch.float32:
                print(f"   ✅ Correct precision: float32")
            else:
                print(f"   ❌ Wrong precision: {processed_audio.dtype} (expected float32)")
            ...
            if 0.95 <= length_ratio <= 1.05:  # Allow 5% tolerance
                print(f"   ✅ Length correct: {actual_length} samples (ratio: {length_ratio:.3f})")
            else:
                print(f"   ⚠️  Length unexpected: {actual_length} samples (expected ~{expected_length}, ratio: {length_ratio:.3f})")
    ...
    try:
        ...
        print(f"\n✅ audiostretchy library is available")
    except ImportError:
        print(f"\n⚠️  audiostretchy library not available - test results may not be representative")
...
def test_write_read_precision():
    ...
    try:
        ...
        if audio_read.dtype == np.float32:
            print(f"✅ soundfile maintains float32 precision")
        else:
            print(f"❌ soundfile changed precision to {audio_read.dtype}")
